...
```python
from tng_control.config.config_loader import ConfigLoader
...
```bash
python -m tng_control.config.generate_schema
...
```bash
python -m tng_control.config.validate_yaml config/configs/gr00t_so101.yaml
```

Validate all configs in a directory:

```bash
python -m tng_control.config.validate_yaml -d config/configs/
```
...
{
  "yaml.schemas": {
    "tng_control/config/rfm_config_schema.json": "config/configs/*.yaml"
  }
}
```
